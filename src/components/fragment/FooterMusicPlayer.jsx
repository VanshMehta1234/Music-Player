...
import { AddBoxOutlined, AddCircleOutlineOutlined, PlaylistPlayOutlined } from "@material-ui/icons";
import AddIcon from "@material-ui/icons/Add";
import axios from 'axios';

function FooterMusicPlayer({ music }) {
    const [{ id, name, artist, img, musicName }, setCurrTrack] = useState(
        music || {}
    );
    ...
    const [isPlaylistOptionsOpen, setPlaylistOptionsOpen] = useState(false);
        ...
        <div style={useStyle.component} className={"footer-player"}>
            ...
            >
                <div className="curr-music-details">
                    <Name name={name} className={"song-name"} length={name ?name.length :0} />
                    <Name name={artist} className={"author-name"} length={artist ?artist.length : 0} />
                </div>
            </Button>
    ...
    <div style={{ position: 'relative' }}>
        <ControlsToggleButton
            style={pointer}
            type={"playlist"}
            defaultIcon={<PlaylistPlayOutlined fontSize={"large"} />}
            changeIcon={<PlaylistPlayOutlined fontSize={"large"} />}
            onClicked={() => handleToggle("playlist", !isPlaylistOptionsOpen)}
        />

        {isPlaylistOptionsOpen && (
            <div className="playlist-options">
                {playlists.map((playlist) => (
                    <div key={playlist.id} className="playlist-options-item" onClick={() => { setSelectedPlaylist(playlist); addToPlaylist(playlist); }}>
