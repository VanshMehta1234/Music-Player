...
public class SongController {
    ...
    public Song addSongToPlaylist(@PathVariable Long playlistId, @RequestBody Song song) {
        // Check if a song with the same name already exists in the playlist
        Optional<Song> existingSong = songRepository.findByNameAndPlaylistsId(song.getName(), playlistId);
        if (existingSong.isPresent()) {
            // Song with the same name already exists in the playlist, return it instead of creating a new one
            return existingSong.get();
        } else {
            // Song with the same name does not exist in the playlist, add it to the playlist
